...
class OptionServiceImpl(OptionService):
    async def get_options(self, es_client: AsyncElasticsearchClient):
        get_unique_categories_body = {
            "id": "get_unique_fields",
            "params": {
                "field": "major"
            }
        }
        ...
        result = await es_client.search(get_unique_categories_body, Index.CATEGORY)
        buckets = result["aggregations"]["unique_fields"]["buckets"]
        majors = [bucket["key"] for bucket in buckets]

        category_result = await es_client.search(get_all_body, Index.CATEGORY)
